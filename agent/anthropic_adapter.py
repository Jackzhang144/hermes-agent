...
def _is_deepseek_anthropic_endpoint(base_url: str | None) -> bool:
    ...
    Per DeepSeek's published compatibility matrix the blocks were originally
    unsigned (no Anthropic-proprietary signature, no ``redacted_thinking``
    support), but 2026-05+ responses include UUID-format signatures.
    This endpoint is handled with a UUID-aware variant of the strip-signed /
    keep-unsigned policy: blocks signed by DeepSeek (UUID format) are
    preserved, Anthropic-signed blocks (base64) are stripped.  The match is pinned to
...
... [OUTPUT TRUNCATED - 48567 chars omitted out of 98567 total] ...
...
def _manage_thinking_signatures(
    ...
    endpoints speak the Anthropic protocol upstream but require thinking
    blocks to round-trip on replayed assistant tool-call messages.
    DeepSeek's blocks carry UUID-format signatures (2026-05+) that are
    distinguished from Anthropic's base64 signatures.  See hermes-agent#13848
    (Kimi) and hermes-agent#16748 (DeepSeek).
